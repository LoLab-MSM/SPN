...
'''
from SPN_mass_action import model
import re

#print (model.odes.elements())
#sys.exit(0)

tspan = np.linspace(0, 1000, 501)
x = odesolve(model, tspan, verbose=False, integrator_options = {'atol' : 1e-6, 'rtol' : 1e-6})

d = {}
for x,i in enumerate(model.species):
#    print (x,i)
    tot = str(i).split(' % ')
    endstr = ''
    for j in tot:
...
for i in d:
    ...
    #print (('__s'+str(i)+'*'))
    #print ((str(d[i]) + '*'))
    str1= re.sub(('__s'+str(i)+'\*'),(str(d[i])+'*'),str1)
    #break
#    print (' ')
print (str1)
sys.exit(0)
